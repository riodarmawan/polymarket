...
impl SignalEngine {
    ...
    fn detect_regime(&self, candles: &[Candle]) -> Regime {
        ...
        let returns: Vec<f64> = candles[n-20..].windows(2)
    ...
    fn generate_momentum_signal(
        ...
        let direction = if all_up { Direction::Up } else { Direction::Down };
    ...
    fn generate_mean_reversion_signal(
        ...
        let recent_mean: f64 = candles[n-lookback..n].iter().map(|c| c.close).sum::<f64>() / lookback as f64;
        let current_price = candles[n-1].close;
        ...
        Some(Signal {
            ...
            reason: format!("TF:{} RSI:{:.1} Dev:{:.4} Ranging", target.as_str(), rsi, deviation),
        })
    }
}
