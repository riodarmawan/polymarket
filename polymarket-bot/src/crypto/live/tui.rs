...
impl TuiRenderer {
    ...
    fn render_price(&self, frame: &mut Frame, area: Rect) {
        let arrow = if self.price_change_pct >= 0.0 {
            "\u{25B2}"
        } else {
            "\u{25BC}"
        };
        let color = if self.price_change_pct >= 0.0 {
            Color::Green
        } else {
            Color::Red
        };
        ...
        let price_line = Line::from(vec![
            ...
            Span::raw("  "),
            Span::styled(
                format!("{} {:+.2}%", arrow, self.price_change_pct),
        ...
        let price_widget = Paragraph::new(price_line).block(
            Block::default()
                .borders(Borders::ALL)
    ...
    fn render_markets(&self, frame: &mut Frame, area: Rect) {
        ...
        if self.markets.is_empty() {
            ...
            for market in self.markets.iter().take(3) {
                ...
                lines.push(Line::from(vec![
                    Span::raw("  "),
                    Span::styled(
                        format!("[{}]", market.question.chars().take(20).collect::<String>()),
                        Style::default()
                            .fg(Color::Yellow)
                            .add_modifier(Modifier::BOLD),
                    ),
                    ...
                    Span::styled(
                        format!("${:.3}", yes),
                        Style::default().fg(yes_color).add_modifier(Modifier::BOLD),
                    ),
                    Span::raw("  NO: "),
                    Span::styled(
                        format!("${:.3}", no),
                        Style::default().fg(no_color).add_modifier(Modifier::BOLD),
                    ),
        ...
        let market_widget = Paragraph::new(lines).block(
            Block::default()
                .title("Market")
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Cyan)),
    ...
    fn render_signal(&self, frame: &mut Frame, area: Rect) {
        ...
        let signal_widget = Paragraph::new(signal_text).block(
            Block::default()
                .title("Signal")
                .borders(Borders::ALL)
    ...
    fn render_open_position(&self, frame: &mut Frame, area: Rect, trade: &Option<Trade>) {
        ...
        let pos_widget = Paragraph::new(pos_text).block(
            Block::default()
                .title("Open Position")
                .borders(Borders::ALL)
    ...
    fn render_stats(&self, frame: &mut Frame, area: Rect, stats: &Stats) {
        ...
        let dd_color = if stats.max_drawdown > 0.25 {
            Color::Red
        } else {
            Color::Yellow
        };
        ...
        let stats_text = vec![
            Line::from(vec![
                ...
                Span::styled(
                    format!(
                        "${:+.2} ({:+.1}%)",
                        stats.total_pnl,
                        (stats.total_pnl / stats.current_capital) * 100.0
                    ),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!("{:.1}%", stats.max_drawdown * 100.0),
                    Style::default().fg(dd_color),
                ),
                Span::raw(format!(
                    "  PF: {:.2}  Avg Win: ${:.2}  Avg Loss: ${:.2}",
    ...
    fn render_trade_history(&self, frame: &mut Frame, area: Rect, trades: &[Trade]) {
        ...
        for trade in recent {
            let (status_icon, pnl_text) = match trade.status {
                ...
                TradeStatus::Closed | TradeStatus::Timeout => match trade.pnl {
                    Some(pnl) => {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!(
                        "{:?} @ ${:.2} -> {}",
                        trade.direction, trade.entry_price, exit_str
                    ),
        ...
        let history_widget = Paragraph::new(lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::DarkGray)),
        );
    ...
    fn render_footer(&self, frame: &mut Frame, area: Rect) {
        ...
        let footer = Paragraph::new(Line::from(vec![Span::styled(
            format!(
                "  Uptime: {}h {}m {}s    Press Ctrl+C to exit",
